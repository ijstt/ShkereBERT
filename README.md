...
## Документация
- **`docs/ПОСОБИЕ.md`** — объяснение технологий (RAG, эмбеддинги, FAISS, BERT, Qwen, метрики) для команды.
- **`docs/ЗАПУСК.md`** — установка, список моделей для загрузки, команды запуска.
...
```
См. подробности и список моделей в `docs/ЗАПУСК.md`.
