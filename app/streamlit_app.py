...
with st.sidebar:
    ...
    source_mode = st.radio("Что используем?", ["Демо: SQuAD v2", "Свой файл (PDF/TXT)"])
    pipe = None
    if source_mode == "Демо: SQuAD v2":
    ...
    st.header("Режим ридера")
    reader_mode = st.radio(
        "Как формировать ответ?",
        ["extractive", "generative", "hybrid"],
        help="extractive — BERT-span (быстро); generative — Qwen (естественный ответ+цитаты); "
             "hybrid — Qwen + проверка обоснованности BERT-ом.",
    )
    if reader_mode in ("generative", "hybrid"):
        st.caption("⏳ Первый запрос грузит локальную Qwen (~несколько секунд).")
...
if question:
    with st.spinner(f"Отвечаю ({reader_mode})..."):
    ...
    st.caption(f"[{ans.mode}] {conf}{ans.reason}")
